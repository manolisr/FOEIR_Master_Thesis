...
#from t_2 import boot, input_data
...
def boot(path,v):
    ...
    # r_3 = pd.concat([r_1, r_2]).drop_duplicates().reset_index(drop=True)
    # r_4 = list(r_3.userid.unique())#r_3['userid'].drop_duplicates().reset_index(drop = True) # list(r_3.userid.unique())
    # r_5 = r_3['itemid'].drop_duplicates().reset_index(drop = True)
    dict = {}
...
def user_feedback(fair_algo, t, u):
    #algo_name = ['FOEIR-DIC']#['FOEIR-DIC', 'FOEIR-DPC', 'FOEIR-DTC']
...
"""Run the above code for the three fair_algo and then need to read, remove duplicate and write the train-1.txt again!"""
...
V = 10 #Number of items recommended to user @Bootstrap phase.
    ...
    #path_list = ['./rankings/FOEIR-DIC/t_', './rankings/FOEIR-DPC/t_', './rankings/FOEIR-DTC/t_']
    #fair_algo = ['FOER-DIC', 'FOEIR-DPC', 'FOEIR-DTC']
    #pathlib.Path.unlink('data/train.txt')
...
# test_1 = pd.read_csv('scoredDataSets/ML1M/t_2/train-1.txt', names=['userid', 'itemid', 'rate'], sep='\t')
# test_1 = test_1.drop_duplicates(keep='first')
#test_1.to_csv('scoredDataSets/ML1M/t_2/train-1.txt', header=None, index=None, sep='\t', mode='w')
